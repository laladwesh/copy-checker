    ...
    <div className="p-8 space-y-8 bg-gray-50 min-h-screen font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-8 rounded-xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 flex flex-col items-center justify-center text-center border border-gray-100">
          <ClipboardDocumentListIcon className="h-16 w-16 text-indigo-500 mb-4" />
          <h2 className="text-2xl font-bold text-gray-800 mb-3">Pending Copies</h2>
          <p className="text-gray-600 mb-6">Copies assigned to you that are awaiting evaluation.</p>
          <button
        ...
        <div className="bg-white p-8 rounded-xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 flex flex-col items-center justify-center text-center border border-gray-100">
          <ClockIcon className="h-16 w-16 text-blue-500 mb-4" />
          <h2 className="text-2xl font-bold text-gray-800 mb-3">Checked History</h2>
          <p className="text-gray-600 mb-6">Review copies you have already evaluated.</p>
          <button
        ...
        <div className="bg-white p-8 rounded-xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 flex flex-col items-center justify-center text-center border border-gray-100">
          <QuestionMarkCircleIcon className="h-16 w-16 text-yellow-500 mb-4" />
          <h2 className="text-2xl font-bold text-gray-800 mb-3">Manage Queries</h2>
          <p className="text-gray-600 mb-6">Respond to student queries about their marked copies.</p>
          <Link
            to="/examiner/queries" // Link to the new ExaminerQueries page
            className="w-full bg-yellow-600 hover:bg-yellow-700 text-white font-semibold py-3 px-6 rounded-lg shadow-md hover:shadow-lg transition duration-200 text-lg flex items-center justify-center"
          >
      ...
      >
        {pending.length === 0 ? (
          <p className="text-gray-600 text-center py-4">You have no pending copies to evaluate. Great job!</p>
        ) : (
      ...
      >
        {history.length === 0 ? (
          <p className="text-gray-600 text-center py-4">No evaluated copies in your history.</p>
        ) : (
          <div className="overflow-x-auto max-h-96 overflow-y-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                  ...
                  <tr key={c._id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        c.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                      }`}>
                        {c.status}
                      </span>
                    </td>
