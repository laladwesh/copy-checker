...
export default function ScanCopyUploadModal({ isOpen, onClose, onUploadSuccess, questionPapers, students }) {
  ...
  const handleUpload = async (e) => {
    ...
    selectedFiles.forEach(file => {
      formData.append('scannedFiles', file); // 'scannedFiles' must match multer field name in backend
    });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Scan & Upload Answer Copy">
      <form onSubmit={handleUpload} className="space-y-6 p-2">
        <div>
          <label htmlFor="studentEmail" className="block text-gray-700 text-base font-medium mb-2">
            Student Email:
          </label>
          <select
            id="studentEmail"
        ...
        <div>
          <label htmlFor="questionPaper" className="block text-gray-700 text-base font-medium mb-2">
            Question Paper:
          </label>
          <select
            id="questionPaper"
        ...
        <div>
          <label htmlFor="scannedFiles" className="block text-gray-700 text-base font-medium mb-2">
        ...
        {uploadMessage && (
          <div className={`p-3 rounded-lg text-sm ${
            uploadMessage.includes('successful') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
          }`}>
            {uploadMessage}
          </div>
        )}
