...
async function extractPageAsImage(pdfBuffer, pageNumber, uniqueId) {
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
            execFile(
                'pdftoppm',
                ['-f', pageNumber.toString(), '-l', pageNumber.toString(), '-png', tmpPdfPath, outputPrefix],
                (err, stdout, stderr) => {
                    ...
                    if (err) {
                        ...
                        if (stderr.includes("MiKTeX requires Windows 10")) {
                            return reject(new Error('pdftoppm failed. MiKTeX compatibility issue detected. Please check your Poppler/MiKTeX installation.'));
                        }
                        return reject(new Error('pdftoppm conversion failed. Check server console for full output.'));
                    }
                    resolve();
                }
            );
        ...
        if (await fs.access(expectedImgPath).then(() => true).catch(() => false)) {
            ...
            if (await fs.access(possibleUnpaddedPath).then(() => true).catch(() => false)) {
                actualImgPath = possibleUnpaddedPath;
            } else {
                const possibleSimplePath = `${outputPrefix}.png`; // Try this as a last resort
                 if (await fs.access(possibleSimplePath).then(() => true).catch(() => false)) {
        ...
        if (!actualImgPath) {
            ...
            console.error(`No expected image file found. Files in temp dir starting with "${uniqueId}_page_":`);
            filesInTemp.filter(f => f.startsWith(`${uniqueId}_page_`)).forEach(f => console.error(`  - ${f}`));
        ...
        try {
            ...
            console.warn(`Cleanup warning: Could not delete temp PDF file ${tmpPdfPath}:`, cleanupErr);
        }
        if (actualImgPath) {
            try {
                if (await fs.access(actualImgPath).then(() => true).catch(() => false)) {
                    await fs.unlink(actualImgPath);
                    console.log(`Deleted temp image: ${actualImgPath}`);
                }
            } catch (cleanupErr) {
                console.warn(`Cleanup warning: Could not delete temp image file ${actualImgPath}:`, cleanupErr);
            }
        }
    }
}
...
router.get('/page-image/:originalFileId/:pageNumber', async (req, res, next) => {
    try {
        ...
        const paperExists = await Paper.exists({ 'driveFile.id': originalFileId });
        const copyExists = await Copy.exists({ 'driveFile.id': originalFileId });

        if (!paperExists && !copyExists) {
            return res.status(404).json({ error: 'Original PDF file not found in database.' });
        }
        ...
        const imageBuffer = await extractPageAsImage(pdfBuffer, parseInt(pageNumber), originalFileId);
        ...
        res.status(500).json({ error: err.message || 'Failed to serve page image. Check server logs for details.' });
    }
});
