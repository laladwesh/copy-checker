...
const initializeScheduledJobs = () => {
  ...
  // Job 5: Daily cache cleanup at 2 AM to prevent disk accumulation
  cron.schedule('0 2 * * *', async () => {
    console.log('[SCHEDULED JOB] Running daily PDF cache cleanup...');
    try {
      const { cleanupOldCachedPDFs } = require('./controllers/admin.controller');
      const result = await cleanupOldCachedPDFs(48); // Delete files older than 48 hours
      console.log(`[SUCCESS] Cache cleanup completed: ${result.deleted} files deleted, ${result.errors} errors`);
    } catch (error) {
      console.error('[ERROR] Error in scheduled cache cleanup:', error);
    }
  });
  ...
  console.log('   - PDF cache cleanup: 2 AM daily (keeps files < 48 hours)');
};
